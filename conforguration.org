...
I run two [[https://www.torproject.org/][Tor]] relays (not exit points).  These scripts let me update and run them—see my post [[https://www.miskatonic.org/2024/04/07/how-i-set-up-a-tor-bridge/][How I set up a Tor bridge]] for more.  The necessary =torrc= is not covered here; it requires special care to configure.
...
Now start a new session with three windows in it: one running Tor, one running the =speedometer= visualization, and one picking out bandwidth notices in the log file.  Being able to script Tmux like this makes it very powerful.
...
** shell4
:PROPERTIES:
:header-args: :var hostname="shell4"
:END:
