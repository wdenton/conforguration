...
You can't build anything from source without making sure all the necessary utilities and libraries are there first.  Once done, this will never need to be run again.  (Unless the version of GCC changes, in which case a new =libgccjit-xx-dev= will be needed.  Check the current version with =gcc --version=.)
...
#+begin_src shell :tangle conforg/scripts/emacs-update-autocompilation.sh :shebang "#!/bin/bash"
cd /usr/local/src/emacs/
git pull
