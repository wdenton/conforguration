...
This only needs to be done once per machine, to create =/usr/local/src/= and make it owned by oneself (on Debian-like systems, at least). This is where all source code will be downloaded and compiled.

#+begin_src shell :tangle conforg/scripts/initialize.sh :shebang "#!/bin/bash"
sudo mkdir -p /usr/local/src/
sudo chown $USER:$USER /usr/local/src
mkdir -p ~/conforg/dotfiles/mine/ ~/conforg/dotfiles/imported/ ~/conforg/scripts/
#+end_src
...
Gather all the dot files and the install script into the =~/conforg/dotfiles/= directory.  It's easiest to have them all there because they can then be pushed to other machines, instead of trying to use Conforguration over there.  Of course it's possible to run Conforguration on another machine and treat it as localhost when logged in there, instead of pushing to it.
...
#+begin_src shell :tangle install/copy-dotfiles.sh :shebang "#!/bin/bash" :eval no
cd "$(dirname "$0")"
#+end_src

Then copy all the files I want from Conforguration into =~/conforg/dotfiles/=.

#+begin_src shell :tangle install/copy-dotfiles.sh :shebang "#!/bin/bash" :eval no
cd ../conforg/
cp -pr dotfiles/* ~/conforg/dotfiles/
#+end_src
...
*** Script to copy the scripts

#+begin_src shell :tangle install/copy-scripts.sh :shebang "#!/bin/bash" :eval no
cd "$(dirname "$0")"
#+end_src
...
#+begin_src shell
rsync --archive --compress --delete ~/conforg/ ${hostname}:conforg/
ssh ${hostname} "conforg/dotfiles/link-dotfiles.sh"
#+end_src

** Install!

The script can be run here or from the command line (called from anywhere---you don't need to be in the same directory).  It puts the current versions of dot files and scripts into =~/conforg/= and then links to the dot files.  After this is done, everything is ready to be copied to other machines.

#+begin_src shell :dir install/ :tangle install/install.sh :shebang "#!/bin/bash" :results silent
cd "$(dirname "$0")"
./copy-dotfiles.sh
...
I run two [[https://www.torproject.org/][Tor]] relays (guards, not exit points).  These scripts let me update and run them, but this is not good enough for someone else to set up a new relay of their own.  I may add more later.
...
Now start a new session with two windows in it.
