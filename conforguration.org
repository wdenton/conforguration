...
By default, =speedometer= will show traffic on the =wlan0= interface.  To use another one, set the TOR_IFACE environment variable in =~/.bash.machine.rc= (using the hostname of the machine), like so: =export TOR_IFACE=ens160= or whatever.
...
#+begin_example
@reboot source ~/.bash.machine.rc; ~/conforg/scripts/tor-run.sh
#+end_example
