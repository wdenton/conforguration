...
Set up =fd= (which on Ubuntu is used by another application) for [[https://github.com/sharkdp/fd][fd]] the replacement for =find=.
...
*** lintr (=.lintr=)
:PROPERTIES:
:header-args: :tangle conforg/dotfiles/mine/lintr
:END:

In my world tabs are four spaces, so I need to specify this for [[https://github.com/r-lib/lintr][lintr]], which thinks they should be two (!?).
...
# In 2022 I made this note, but I don't know why.
...
#+begin_src shell :tangle conforg/scripts/emacs-update-no-native-compilation.sh :shebang "#!/bin/bash"
cd /usr/local/src/emacs/
git pull
./configure --with-json --with-pgtk --with-tree-sitter && make -j && cd ../org-mode/ && make -j update
#+end_src

#+begin_src shell :tangle conforg/scripts/emacs-update-native-compilation.sh :shebang "#!/bin/bash"
cd /usr/local/src/emacs/
git pull
./configure --with-json --with-pgtk --with-tree-sitter --with-native-compilation=aot && make -j && cd ../org-mode/ && make -j update
#+end_src

I want ahead-of-time native compilation to be the default, so I set this.  The other script is always available, and this symlink can be changed as needed.

#+begin_src shell :dir conforg/scripts/
rm -f emacs-update.sh
ln -s emacs-update-native-compilation.sh emacs-update.sh
#+end_src
...
#+RESULTS: packages-list-variable
...
I run a [[https://www.torproject.org/][Tor]] relay (a guard, not an exit point).  These scripts let me update and run it, but this is not good enough for someone else to set up a new relay of their own.  I may add more later.
