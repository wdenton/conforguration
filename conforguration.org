...
Repeat a command n times, for example ~repeat 5 echo "Hello"~ .  When repeating multiple commands use quoting to make sure parts don't get lost, for example ~repeat 5 echo "Hello" ";" echo "Goodbye"~.
...
*** Config (=.sqliterc=)
:PROPERTIES:
:header-args: :tangle conforg/dotfiles/mine/sqliterc
:END:
...
mkdir -p /usr/local/src/tor/ /usr/local/src/tor/log/
#+end_src
