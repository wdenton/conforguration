...
First get the source for Emacs and compile it, then get the source for Org and compile it.  Download my Emacs configuration and run the special script that just installs [[https://github.com/jwiegley/use-package][use-package]], with is enough to let my =init.el= install everything else I want.
