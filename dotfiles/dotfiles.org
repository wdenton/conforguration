...
Note about languages specified in the source blocks:  the language setting is required, but it doesn't have to be a known and supported language.  So in order for the tmuxrc to be tangled, for example, I say it has the language "tmux," which doesn't exist but makes it work.

TODO:  shellcheck the scripts?
...
#+RESULTS:
| mine/xmodmaprc | mine/tmux.conf | mine/signature.work | mine/signature | mine/pryrc | mine/rubocop.yml | mine/gemrc | mine/Rprofile | mine/nanorc | mine/gitconfig | mine/abcde.conf | mine/lessfilter | mine/bash_logout | mine/bashrc | mine/bash_profile | install-dotfiles.sh |

** Create the install script

Now we can make the install script, which can be run from anywhere.  Don't run it yet---that comes after one more step.

First, move into the directory where the script is.  There is [[https://stackoverflow.com/questions/59895/how-can-i-get-the-source-directory-of-a-bash-script-from-within-the-script-itsel][much discussion on Stack Overflow]] about the best way to do this, but this works.

#+begin_src shell :tangle install-dotfiles.sh :shebang "#!/bin/bash" :eval no
cd "$(dirname "$0")"
# echo "PWD is " $(pwd)
#+end_src
...
Gather all the files into the =~/dotfiles/= directory.  It's easiest to have them all there because they can then be pushed to other machines, instead of trying to use Conforguration over there.

#+begin_src shell :tangle gather-dotfiles.sh :shebang "#!/bin/bash" :eval no
cd "$(dirname "$0")"
...
* Abcde
:PROPERTIES:
:header-args: :tangle mine/abcde.conf
:END:
...
:PROPERTIES:
:header-args: :tangle mine/bash_profile
:END:
...
** Config (=.bashrc=)
:PROPERTIES:
:header-args: :tangle mine/bashrc
:END:
...
** Logout (=.bash_logout=)
:PROPERTIES:
:header-args: :tangle mine/bash_logout
:END:
...
** Config (=.gitconfig=)
:PROPERTIES:
:header-args: :tangle mine/gitconfig
:END:
...
** Lessfilter
:PROPERTIES:
:header-args: :tangle mine/lessfilter :shebang "#!/bin/bash"
:END:
...
* Nano (=.nanorc=)
:PROPERTIES:
:header-args: :tangle mine/nanorc
:END:
...
** Rprofile (=.Rprofile=)
:PROPERTIES:
:header-args: :tangle mine/Rprofile
:END:
...
** Rubocop (=.rubocop.yml=)
:PROPERTIES:
:header-args: :tangle mine/rubocop.yml
:END:
...
** Personal
:PROPERTIES:
:header-args: :tangle mine/signature
:END:
...
** Work
:PROPERTIES:
:header-args: :tangle mine/signature.work
:END:
...
* Tmux (=.tmux.conf=)
:PROPERTIES:
:header-args: :tangle mine/tmux.conf
:END:
...
** Xmodmap (=.xmodmaprc=)
:PROPERTIES:
:header-args: :tangle mine/xmodmaprc
:END:
